...
from disco.core.latent_diffusion.data.datasets import InpaintDataset, OutpaintDataset, Slice3DDataset
...
def build_inpaint_dataset(
    ...
    This expects ``root_dir`` to contain ``train`` and ``val`` subdirectories.
    ...
    Args:
        root_dir: Root directory containing the dataset splits.
...
def build_outpaint_dataset(
    ...
    Raises:
        ValueError: If ``root_dir`` is not provided.
    """
    if not root_dir:
...
def build_three_dim_dataset(
    ...
    Args:
        root_dir: Root directory containing the 3D slice data.
