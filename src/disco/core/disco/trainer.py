...
class DiscoTrainer:
    ...
    def train(
        ...
        *,
        ...
        if verbose:
            print(
                f"[DiscoTrainer] Starting training pipeline "
                f"(device={resolved_device}, dtype={resolved_dtype}, seed={seed})."
            )

        preprocess_config = preprocess_config or PreProcessConfig()
        processed_df, _ = PreProcessor(config=preprocess_config).preprocess(df=df, verbose=verbose)

        if gcnn_artifact is None:
            if verbose:
                print("[DiscoTrainer] Training GCNN component.")
            ...
            print("[DiscoTrainer] Using provided GCNN artifact.")
        ...
        if autoencoder_artifact is None:
            if verbose:
                print("[DiscoTrainer] Training autoencoder component.")
            ...
            print("[DiscoTrainer] Using provided autoencoder artifact.")
        ...
        resolved_latent_strategy = DiscoTrainer._resolve_strategy(
        ...
        if latent_diffusion_artifact is None:
            if verbose:
                print("[DiscoTrainer] Training latent diffusion component.")
            ...
            print("[DiscoTrainer] Using provided latent diffusion artifact.")

        if pixel_diffusion_artifact is None:
            expected_precompute_strategy_cls = PRECOMPUTE_STRATEGY_REGISTRY[inference_mode]
            resolved_precompute_strategy = DiscoTrainer._resolve_strategy(
            ...
            if verbose:
                print("[DiscoTrainer] Precomputing pixel diffusion dataset.")
            ...
            train_index, val_index = pd_precomputer.precompute(
                root_dir=Path(pd_precomputer_root_dir) if pd_precomputer_root_dir else root_dir,
                out_dir=Path(pd_precomputer_out_dir) if pd_precomputer_out_dir else root_dir,
                batch_size=pd_precomputer_batch_size,
                num_workers=pd_precomputer_num_workers,
            )

            if verbose:
                print("[DiscoTrainer] Training pixel diffusion component.")
            ...
            print("[DiscoTrainer] Using provided pixel diffusion artifact.")

        if verbose:
            print("[DiscoTrainer] Finished training pipeline.")
