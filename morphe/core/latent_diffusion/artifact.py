...
class LatentDiffusionArtifact:
    ...
    img_size: int | None = (
        None  # Optional image size metadata for inference preprocessing (only inpainting/outpainting)
    )
    ...
    def save(self, path: str | Path) -> None:
        ...
        payload: dict[str, Any] = {
            ...
            "cond_encoder_state_dict": self._state_dict_to_cpu(
                self.cond_encoder_state_dict
            ),
            "coord_encoder_state_dict": self._state_dict_to_cpu(
                self.coord_encoder_state_dict
            ),
            "bbox_encoder_state_dict": self._state_dict_to_cpu(
                self.bbox_encoder_state_dict
            ),
    ...
    def load(cls, path: str | Path) -> "LatentDiffusionArtifact":
        ...
        if not isinstance(payload, dict):
            raise ValueError(
                "Invalid LatentDiffusionArtifact file: payload must be a dictionary."
            )
